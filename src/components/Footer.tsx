...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem("cryptohunter-visitors");
    const count = stored ? parseInt(stored) : 1000;
    const newCount = count + 1;
    setVisitorCount(newCount);
    localStorage.setItem("cryptohunter-visitors", newCount.toString());
  }, []);
    ...
    <footer className="border-t border-border mt-20">
      <div className="container mx-auto px-4 py-8">
        <div className="text-center text-muted-foreground">
          <p className="font-semibold text-lg mb-2">CryptoHunter.top</p>
          <p>&copy; 2025 CryptoHunter.top - All Rights Reserved</p>
          <p className="text-sm mt-2">
            The Best Crypto Airdrops for DeFi, NFT, Gaming and more
          </p>
          <p className="text-xs mt-4 text-muted-foreground/50">
            Disclaimer: CryptoHunter.top provides information about airdrops. 
